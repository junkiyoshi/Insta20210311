...
void ofApp::setBoxToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float height, float width, float depth) {
	...
	for (int i = index; i < face_target.getNumVertices(); i++) {
		...
		auto noise_x = ofMap(ofNoise(face_target.getVertex(i).y * 0.005 + ofGetFrameNum() * 0.01), 0, 1, -150, 150);

		face_target.setVertex(i, face_target.getVertex(i) + glm::vec3(noise_x, 0, 0));
		frame_target.setVertex(i, frame_target.getVertex(i) + glm::vec3(noise_x, 0, 0));
	}
}
